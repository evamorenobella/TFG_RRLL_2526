# Bienvenidos/as al repositorio de [TFG](https://laborales.ugr.es/docencia/trabajo-fin-grado) (Grado en RRLL-RRHH) de la profesora Eva Moreno Bella 
</head>
<body>
  <main>
    <section>
      <h2>Breve descripción de contenidos</h2>
      <p>
        De acuerdo con los objetivos y contenidos establecidos en las materias de carácter obligatorio y formación básica, los estudiantes deberán realizar, presentar y defender un trabajo-memoria en el que demuestren las competencias específicas asociadas al título, adquiridas a lo largo de los cuatro años de estudios. La dirección y tutorización de estos trabajos se realizará por profesorado universitario.
      </p>
      <section>
        <section>
      <h2>Tipologías de TFG</h2>
      <ul>
        <li>
          <strong>TFG estándar o monográfico:</strong>
          Es un TFG que va a consistir en una memoria que debe contemplar una revisión de la literatura de algún tema/campo que integra el Grado. En este caso, la revisión debe hacerse de alguno de los siguientes temas: Desigualdad de género y liderazgo en las organizaciones o Deshumanización organizacional.
        </li>
        <li>
          <strong>TFG asociado a un portafolio de competencias:</strong>
          Es un TFG en el que el alumno debe demostrar mediante una memoria en la que incluya una lista de evidencias, que ha adquirido una serie de competencias fundamentales para su Grado. Dichas evidencias pueden ser trabajos escritos realizados por el alumno, ya sean individuales o grupales, videos, etc. Este portafolio debe reflejar el aprendizaje realizado por el alumno a lo largo de sus estudios en Relaciones Laborales y Recursos Humanos.
        </li>
      </ul>
      <p>
        El TFG tendrá un máximo de 5500 palabras quedando excluidos de este cómputo la primera página, bibliografía o notas al pie.
      </p>
       <section>
      <h2>Defensa del TFG</h2>
      <p>
        El TFG se defenderá ante el tutor que incluirá dicha defensa en la calificación final que otorgue al estudiante. Solamente los estudiantes propuestos a Matrícula de Honor, defenderán el TFG ante un Tribunal.
      </p>
      <p>Debéis preparar una presentación de 10 minutos como máximo.</p>
      <section>
      <h2>Uso de IA</h2>
      <p>
        Tened cuidado con el uso de IA en vuestros trabajos; debe ser una herramienta y que vuestro tutor conozca qué partes de vuestro TFG están apoyados con IA. En
        <a href="https://laborales.ugr.es/docencia/trabajo-fin-grado/recursos-ayuda" target="_blank" rel="noopener noreferrer">
          https://laborales.ugr.es/docencia/trabajo-fin-grado/recursos-ayuda
        </a>
        tenéis varias guías al respecto.
        <a href="https://laborales.ugr.es/sites/centros/rlaborales/public/ficheros/OA%20y%20TFG/PautasIA.pdf" target="_blank" rel="noopener noreferrer">
          Guía de Buenas Prácticas TFG-IA
          </a>
      </p>
        Asimismo, me debéis entregar la [declaración de uso de IA](https://docs.google.com/document/d/1429HgIiPFjtrnuWjK2_O393YoxhOp4B0/edit?usp=sharing&ouid=101524530347639054232&rtpof=true&sd=true) junto al TFG 
       <section>
      <h2>Criterios de evaluación de los TFG</h2>
      <ul>
        <li>Memoria y trabajo continuo: <strong>60%</strong></li>
        <li>Defensa del TFG: <strong>40%</strong></li>
      </ul>
      <p>
        Una vez el TFG esté terminado (en cualquiera de sus modalidades), el tutor/a comunicará al estudiantado la calificación que va a proponer y que se ponderará con el 100% de la calificación final.
      </p>
      <p>
        <strong>Recursos de Ayuda:</strong>
        <a href="https://laborales.ugr.es/docencia/trabajo-fin-grado/recursos-ayuda" target="_blank" rel="noopener noreferrer">
          https://laborales.ugr.es/docencia/trabajo-fin-grado/recursos-ayuda
        </a>
      </p>
      <section>
      <h2>Plazos de entrega</h2>
      <ul>
        <li>
          <strong>Convocatoria ordinaria.</strong>
          La fecha límite para el envío de calificación por parte del tutor y documentación del TFG por parte del estudiante es el 9 de junio de 2026. La defensa ante el Tribunal de MH o reclamaciones es el 29 de junio de 2026. Toda la información necesaria para la defensa del TFG se comunicará a través de Prado con suficiente antelación. Los alumnos que deseen un tribunal de reclamaciones deben solicitarlo por correo electrónico dentro de los tres días siguientes a la fecha límite
          (<a href="mailto:mdhuete@ugr.es">mdhuete@ugr.es</a>).
        </li>
        <li>
          <strong>Convocatoria extraordinaria.</strong>
          La fecha límite para el envío de calificación por parte del tutor y documentación del TFG por parte del estudiante es el 3 de julio de 2026. La defensa ante el Tribunal de MH o reclamaciones es el 20 de julio de 2026. Toda la información necesaria para la defensa del TFG se comunicará a través de Prado con suficiente antelación. Los alumnos que deseen un tribunal de reclamaciones deben solicitarlo por correo electrónico dentro de los tres días siguientes a la fecha límite
          (<a href="mailto:mdhuete@ugr.es">mdhuete@ugr.es</a>).
        </li>
      </ul>
      <p>
        El TFG debe ser entregado al tutor al menos 15 días antes del plazo de entrega oficial.
      </p>
    <section>
  <h2>Cronograma propuesto para la tutorización de TFG</h2>
  <ol>
    <li>
      <h3>Propuesta inicial</h3>
      <p><strong>Fecha de entrega:</strong> 12 de diciembre</p>
      <p><strong>Monográfico:</strong></p>
      <p>
        Entregar 1-2 páginas con: tema elegido y justificación de por qué lo ha elegido (i.e., breve introducción y contextualización del tema), objetivo general y cinco referencias iniciales para trabajar (en formato APA).
      </p>
      <p><strong>Portafolio:</strong></p>
      <p>
        Entregar 1-2 páginas con: justificación de por qué elige portafolio, autoevaluación muy breve de las 8 competencias, lista provisional de evidencias que cree que puede aportar (trabajos, prácticas, etc.). Creación del Google Site y envío del enlace.
      </p>
    </li>
    <li>
      <h3>Borradores iniciales en la plantilla</h3>
      <p><strong>Fecha de entrega:</strong> 26 de enero</p>
      <p><strong>Monográfico:</strong></p>
      <ul>
            <li>
              Borrador de la introducción y objetivos, así como del apartado de metodología
              (Estrategia de búsqueda bibliográfica,
              <a href="https://biblioteca.ugr.es/biblioteca-electronica/bases-datos" target="_blank" rel="noopener noreferrer">
                bases de datos
              </a>, palabras clave), siguiendo la plantilla estándar.
            </li>
        <li>Lista/tablas con la literatura revisada hasta ahora (autor, año, tipo de estudio, muestra, resultados clave).</li>
      </ul>
      <p><strong>Portafolio:</strong></p>
      <ul>
        <li>Tabla donde se indique, para cada competencia, qué evidencias definitivas se utilizarán (mínimo 1 por competencia).</li>
        <li>Breve descripción de cada una de ellas y por qué las ha elegido.</li>
      </ul>
    </li>
    <li>
      <h3>Avance 1 (aprox. 30-40% del trabajo)</h3>
      <p><strong>Fecha de entrega:</strong> 2 de marzo</p>
      <p><strong>Monográfico:</strong></p>
      <ul>
        <li>Borrador de introducción y marco teórico (unas 2000–2500 palabras).</li>
        <li>Breve esquema de los posibles resultados y/o borrador de los mismos.</li>
        <li>Literatura revisada: como mínimo 10 artículos.</li>
      </ul>
      <p><strong>Portafolio:</strong></p>
      <ul>
        <li>Memoria: redactadas al menos 5 competencias con la descripción completa (logros, puntos fuertes/débiles, justificación, etc.).</li>
      </ul>
      <p><strong>Portafolio digital:</strong></p>
      <ul>
        <li>Site ya estructurado con las 8 competencias.</li>
        <li>Subidas al menos las evidencias de esas 3–4 competencias.</li>
      </ul>
    </li>
    <li>
      <h3>Avance 2 (versión casi completa)</h3>
      <p><strong>Fecha de entrega:</strong> 20 de abril</p>
      <p><strong>Monográfico:</strong></p>
      <ul>
        <li>Borrador casi completo (80 %):</li>
        <li>Introducción y revisión teórica prácticamente cerradas.</li>
        <li>Apartado de “resultados” de la revisión (organización de hallazgos, tablas/figuras si procede).</li>
        <li>Al menos un esquema o borrador de las conclusiones preliminares.</li>
        <li>Referencias.</li>
      </ul>
      <p><strong>Portafolio:</strong></p>
      <ul>
        <li>Memoria: Las 8 competencias ya redactadas.</li>
        <li>Conclusiones preliminares sobre qué ha aprendido en el grado y puntos a mejorar.</li>
      </ul>
      <p><strong>Portafolio digital:</strong></p>
      <ul>
        <li>Todas las evidencias subidas y ordenadas.</li>
        <li>Anexo de ficheros listado en la memoria.</li>
      </ul>
    </li>
    <li>
      <h3>Versión final para corrección (Entrega a la tutora)</h3>
      <p><strong>Fecha de entrega:</strong> 25 de mayo</p>
      <p><strong>Monográfico:</strong></p>
      <p>Versión completa con formato definitivo.</p>
      <p><strong>Portafolio:</strong></p>
      <p>Versión completa con formato definitivo.</p>
      <p><strong>Portafolio digital definitivo.</strong></p>
    </li>
    <li>
      <h3>Defensa</h3>
      <p><strong>Fecha:</strong> Por determinar (finales de mayo, principios de junio).</p>
      <p>Presentación de 10 minutos y debate/preguntas.</p>
      <p>Envío de las diapositivas o material de presentación a la tutora con 24 de antelación a la defensa.</p>
    </li>
  </ol>
</section>
        <section>
  <h2>Fechas importantes</h2>
  <table>
    <caption>Fechas importantes</caption>
    <thead>
      <tr>
        <th>Fecha</th>
        <th>Hito</th>
        <th>Convocatoria</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>12 de diciembre</td>
        <td>Propuesta inicial</td>
        <td>-</td>
      </tr>
      <tr>
        <td>26 de enero</td>
        <td>Borradores iniciales en la plantilla</td>
        <td>-</td>
      </tr>
      <tr>
        <td>2 de marzo</td>
        <td>Avance 1 (30–40% del trabajo)</td>
        <td>-</td>
      </tr>
      <tr>
        <td>20 de abril</td>
        <td>Avance 2 (versión casi completa)</td>
        <td>-</td>
      </tr>
      <tr>
        <td>25 de mayo</td>
        <td>Versión final para corrección (Entrega a la tutora)</td>
        <td>-</td>
      </tr>
      <tr>
        <td>9 de junio de 2026</td>
        <td>Entrega calificación del tutor y documentación del TFG</td>
        <td>Ordinaria</td>
      </tr>
      <tr>
        <td>3 de julio de 2026</td>
        <td>Entrega calificación del tutor y documentación del TFG</td>
        <td>Extraordinaria</td>
      </tr>
    </tbody>
  </table>
  <p><em>El TFG debe ser entregado al tutor al menos 15 días antes del plazo de entrega oficial, para asi evaluarlo y realizar la defensa.</em></p>
  <p><em>Recordad que para llevar un buen seguimiento de vuestro TFG, debéis asistir al menos a 3 tutorías a lo largo del curso</em></p>
</section>
<section>
  <h2>Materiales de Referencia Utilizados</h2>
  <ul>
    <li>Guía docente de la asignatura TFG. Grado en RRLL</li>
    <li>Materiales disponibles en el curso de la asignatura TFG en PRADO.</li>
  </ul>
</section>

<h2>Artículos sobre Deshumanización Organizacional</h2>
<ul>
  <li>
    <a href="https://www.redalyc.org/journal/2313/231369562002/231369562002.pdf" target="_blank" rel="noopener noreferrer">
      Artículo 1
    </a>
  </li>
  <li>
    <a href="https://onlinelibrary.wiley.com/doi/full/10.1002/ejsp.2944" target="_blank" rel="noopener noreferrer">
      Artículo 2
    </a>
  </li>
  <li>
    <a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC4173804/" target="_blank" rel="noopener noreferrer">
      Artículo 3
    </a>
  </li>
  <li>
    <a href="https://bpspsychub.onlinelibrary.wiley.com/doi/10.1111/joop.12493" target="_blank" rel="noopener noreferrer">
      Artículo 4
    </a>
  </li>
</ul>

<h2>Artículos sobre Género y Liderazgo</h2>
<ul>
  <li>
    <a href="https://www.frontiersin.org/journals/psychology/articles/10.3389/fpsyg.2015.01400/full" target="_blank" rel="noopener noreferrer">
      Artículo sobre género y liderazgo 1
    </a>
  </li>
  <li>
    <a href="https://onlinelibrary.wiley.com/doi/abs/10.1002/(SICI)1099-1379(199601)17:1%3C33::AID-JOB778%3E3.0.CO;2-F?casa_token=7Y5-HlITAYoAAAAA:SgKy1YOBwyApCm9Af0TBp5a0IVpwRMOx3QTUQqvBQVPVllZwlbslyzslHLLCHrk4G7tvKvSqwRu5FOQ" target="_blank" rel="noopener noreferrer">
      Artículo sobre género y liderazgo 2
    </a>
  </li>
  <li>
    <a href="https://psycnet.apa.org/record/2010-26139-001" target="_blank" rel="noopener noreferrer">
      Artículo sobre género y liderazgo 3 
    </a>
  </li>
  <li>
    <a href="https://spssi.onlinelibrary.wiley.com/doi/abs/10.1111/0022-4537.00233?casa_token=AaS98HimemQAAAAA:S7bcz7wlgfX8iwqXtNfHyy4_9uPQOl1Pce6ROnegQZQQCEqpGVm-3H1ZRUbuPzhTkzzTO0YHZ7vUR2w" target="_blank" rel="noopener noreferrer">
      Artículo sobre género y liderazgo 4 
    </a>
    <p><em>Recordad que para acceder a algunos artículos necesitaréis estar conectadas a Internet desde la facultad o con la VPN de la UGR si estáis en casa</em></p>
  </li>
</ul>
